...
public:
    ...
    virtual void TearDown() override{
        ...
        stub.reset(ADDR(Settings, getOption));
    }
};


QVariant getOption_gif_stub(void* obj, const QString &key)
{
    if(key == "save_as_gif") {
        return true;
...
QVariant getOption_mp4_stub(void* obj, const QString &key)
{
    if(key == "save_as_gif") {
        return false;
    }else if(key == "save_directory"){
...
QVariant getOption_mkv_stub(void* obj, const QString &key)
{
    if(key == "save_as_gif") {
        return false;
    }else if(key == "mkv_framerate"){
...
{
    stub.set(ADDR(Settings, getOption), getOption_gif_stub);
...
{
    stub.set(ADDR(Settings, getOption), getOption_mp4_stub);
    RecordProcess *m_process;
    m_process = new RecordProcess;
    m_process->setRecordAudioInputType(RecordProcess::RECORD_AUDIO_INPUT_SYSTEMAUDIO);
    m_process->setRecordInfo(QRect(0, 0, 1920, 1080), "test");
...
{
    stub.set(ADDR(Settings, getOption), getOption_mkv_stub);
    RecordProcess *m_process;
    m_process = new RecordProcess;
    m_process->setRecordAudioInputType(RecordProcess::RECORD_AUDIO_INPUT_SYSTEMAUDIO);
    m_process->setRecordInfo(QRect(0, 0, 1920, 1080), "test");
