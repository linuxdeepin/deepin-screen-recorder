...
int EGifGCBToSavedExtension(const GraphicsControlBlock *GCB, GifFileType *GifFile, int ImageIndex)
{
    ...
    {
        ExtensionBlock *ep = &GifFile->SavedImages[ImageIndex].ExtensionBlocks[0];
        if (ep->Function == GRAPHICS_EXT_FUNC_CODE)//0xf9
        {
...
int EGifSpew(GifFileType *GifFileOut)
{
    ...
    SavedImage *sp = &GifFileOut->SavedImages[0];
...
int writeExtensions(GifFileType *GifFileOut)
{
    if (EGifWriteExtensions(GifFileOut,
                            GifFileOut->ExtensionBlocks,
                            GifFileOut->ExtensionBlockCount) == GIF_ERROR)
